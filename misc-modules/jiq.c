...
static int jiq_read_wq(struct seq_file *s, int unused)
{
	DEFINE_WAIT(wait);

	jiq_data.len = 0;                /* nothing printed, yet */
	jiq_data.buf = (void *)s;              /* print in this place */
...
static int jiq_read_wq_delayed(struct seq_file *s, int unused)
{
	DEFINE_WAIT(wait);
...
static int jiq_read_tasklet(struct seq_file *s, int unused)
{
	jiq_data.len = 0;                /* nothing printed, yet */
	jiq_data.buf = (void *)s;              /* print in this place */
	jiq_data.jiffies = jiffies;      /* initial time */
	...
	tasklet_schedule(&jiq_tasklet);
	wait_event_interruptible(jiq_wait, jiq_data.len > LIMIT);    /* sleep till completion */
...
static int jiq_read_run_timer(struct seq_file *s, int unused)
{
	...
	jiq_timer.expires = jiffies + 10; /* Timer for 10 jiffies */
...
static int jiq_show(struct seq_file *s, void *unused)
{
	...
	case JIQ_WQ:
		...
		break;
	case JIQ_DELYAED_WQ:
		jiq_read_wq_delayed(s, 0);
		break;
	case JIQ_TIMER:
		jiq_read_run_timer(s, 0);
		break;
	case JIQ_TASKLET:
		jiq_read_tasklet(s, 0);
		break;
	}
	return 0;
}
...
{
	return single_open(file, jiq_show, PDE_DATA(inode));
}

static const struct file_operations jiq_ops = {
	.owner = THIS_MODULE,
	.open = jiq_open,
	.read = seq_read,
	.llseek = seq_lseek,
...
static int jiq_init(void)
{
	...
	proc_create_data("jiqwq", 0, NULL, &jiq_ops, (void *)JIQ_WQ);
	proc_create_data("jiqwqdelay", 0, NULL, &jiq_ops, (void *)JIQ_DELYAED_WQ);
	proc_create_data("jiqtimer", 0, NULL, &jiq_ops, (void *)JIQ_TIMER);
	proc_create_data("jiqtasklet", 0, NULL, &jiq_ops, (void *)JIQ_TASKLET);
